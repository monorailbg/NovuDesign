...
function Card({ t, i, vis }: { t: typeof testimonials[0]; i: number; vis: boolean }) {
  ...
  return (
    <div
      ...
      style={{
        borderColor: hov ? `${t.color}40` : "rgba(167,139,250,0.1)",
        background: hov ? `${t.color}08` : "rgba(109,40,217,0.04)",
        opacity: vis ? 1 : 0,
    ...
    >
      ...
      <p className="text-sm md:text-base leading-relaxed flex-1" style={{ color: "rgba(196,181,253,0.55)" }}>
        {t.quote}
      </p>

      <div className="flex items-center gap-4 pt-4 border-t" style={{ borderColor: "rgba(167,139,250,0.08)" }}>
        ...
        <div>
          ...
          <p className="text-xs mt-0.5" style={{ color: "rgba(167,139,250,0.4)" }}>{t.title}</p>
        </div>
      </div>
    </div>
  );
        ...
        <div ref={ref}>
          <div style={{ opacity: vis ? 1 : 0, transform: vis ? "translateY(0)" : "translateY(24px)", transition: "all 0.7s ease" }}
            ...
            <div className="flex items-center gap-3 mb-5">
              ...
              <span className="text-xs font-bold tracking-widest uppercase" style={{ color: "#7C3AED" }}>Social Proof</span>
            </div>
            <h2 className="font-heading font-black text-5xl md:text-[80px] text-white leading-[0.9] tracking-tighter">
              Words from<br />
              <span style={{ color: "rgba(167,139,250,0.28)" }}>those who know.</span>
            </h2>
          </div>
